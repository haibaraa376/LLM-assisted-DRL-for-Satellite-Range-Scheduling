...
def run_reward_search(provider_name, baseline_config, mappo_config, output_directory, rounds, candidates_per_round, candidate_episodes, smoke=False, live_api_approval=None):
    ...
    for round_index in range(1, int(rounds) + 1):
        ...
        for candidate_index in range(1, int(candidates_per_round) + 1):
            ...
            prompt = build_initial_reward_prompt(config["training"]["task_count"], {"candidate_id": candidate_id}) if parent is None else build_feedback_reward_prompt(config["training"]["task_count"], parent["candidate_id"], parent)
            spec, audit = generator.generate(prompt, {"candidate_id": candidate_id, "task_count": int(config["training"]["task_count"])})
