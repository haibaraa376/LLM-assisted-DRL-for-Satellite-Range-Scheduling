...
def build_comparison(run_directory, methods, method_states=None):
    ...
    completed = [
        item
        for item in records
        if item.get("timeliness_raw_mean") is not None
    ]
    ranking = sorted(
        completed,
        key=lambda item: (
    ...
    atomic_write_json(root / "comparison.json", payload)
    return payload
